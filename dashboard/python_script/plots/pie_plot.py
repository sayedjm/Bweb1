...
def get_pie_plot(selected_country):
    ...
    covid_data = pd.DataFrame(
        covid_data_objects.values('Confirmed', 'Recovered', 'Deaths'))
    total_values = covid_data.max()
    fig = px.pie(total_values, values=total_values.values,
                 names=total_values.index,
                 title=f'Distribution Confirmed, Recovered and Deaths')
